// =============================================================
// Score should be around 1500, needs types analysis/replacement
// =============================================================
 ...
 *
 * V0 is loaded via ldVXY0 (packed s16 x|y in one 32-bit word) and ldVZ0 (z).
 * The rotation matrix is a 3x3 of s16 (1.3.12 fixed point), stored as
 * MATRIX.m[row][col] with a pad short after each row of 3.
 *
 * Reference: ctr/gte_simulator.h _gte_mvmva()
 */

static MATRIX *_gte_rot;
static short _gte_vx, _gte_vy, _gte_vz;
static int _gte_mac1, _gte_mac2, _gte_mac3;

static void _gte_do_rtv0(void)
{
  /* MATRIX.m is short[3][3] — rows are contiguous, no inter-row padding
     because the struct is: short m[3][3]; short pad; int t[3]; */
  short (*m)[3] = _gte_rot->m;
  _gte_mac1 = ((int)m[0][0] * _gte_vx + (int)m[0][1] * _gte_vy + (int)m[0][2] * _gte_vz) >> 12;
  _gte_mac2 = ((int)m[1][0] * _gte_vx + (int)m[1][1] * _gte_vy + (int)m[1][2] * _gte_vz) >> 12;
  _gte_mac3 = ((int)m[2][0] * _gte_vx + (int)m[2][1] * _gte_vy + (int)m[2][2] * _gte_vz) >> 12;
}

#define gte_SetRotMatrix(r0) do { _gte_rot = (MATRIX *)(r0); } while (0)

#define gte_ldVXY0(r0) do {                    \
  unsigned int _packed = (unsigned int)(r0);    \
  _gte_vx = (short)(_packed & 0xFFFF);         \
  _gte_vy = (short)(_packed >> 16);             \
} while (0)

#define gte_ldVZ0(r0) do { _gte_vz = (short)(r0); } while (0)
