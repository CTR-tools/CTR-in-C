/*
 * UI_DrawNumTrophy @ 0x80051070 (func_80051070, SCUS_944.26 / NTSC-U 926)
 *
 ...
 *
 * Identical shape to UI_DrawNumRelic/UI_DrawNumKey (differs only in the counter
 ...
 *
 * Draws the trophy HUD counter: an "x" icon then the trophy count as a number.
 *   D_8008D528 = sdata->s_x, D_8008D520 = sdata->s_longInt, D_8008D2AC = sdata->gGT
 *   gGT + 0x1E30 = gGT->currAdvProfile.numTrophies
 *   gGT + 0x08   = gGT->gameMode2 (bit 0x4000000 => trophies-incomplete, show count-1)
...
void UI_DrawNumTrophy(int posX, int posY)
{
    ...
    num = *((int *) (D_8008D2AC + 0x1E30));
    textX = posX + 0xD;

    if ((*((uint *) (D_8008D2AC + 8)) & 0x4000000) != 0)
        num = num - 1;
